...
.dkgtipfy {
	...
	> .buttons {
		...
		pointer-events: none; // ボタン削減で空白が出来るのでそこをタッチできるように
		...
		> .button {
			...
			background: var(--panel);
			color: var(--fg);
			box-shadow: 0px 1px 5px var(--shadow); // フローティングっぽく変更
			pointer-events: auto; // 親の.buttonsでnoneにしてるのでボタン部は改めて機能するように再指定

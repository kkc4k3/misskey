<template>
    <div class="mk-user-page" v-if="user" v-size="{ max: [500] }">
        ...
        />
        <div class="punished _panel" v-if="user.isSuspended">
            <fa :icon="faExclamationTriangle" style="margin-right: 8px" />
            {{ $t("userSuspended") }}
        </div>
        <div class="punished _panel" v-if="user.isSilenced">
            <fa :icon="faExclamationTriangle" style="margin-right: 8px" />
            {{ $t("userSilenced") }}
        </div>
        <div class="profile _panel" :key="user.id">
            <div class="banner-container" :style="style">
                ...
                <div class="title">
                    ...
                    <div class="bottom">
                        ...
                        <span
                            ...
                            ><fa :icon="farBookmark"
                        /></span>
                        <span v-if="user.isLocked" :title="$t('isLocked')"
                            ><fa :icon="faLock"
                        /></span>
                        <span v-if="user.isBot" :title="$t('isBot')"
                            ><fa :icon="faRobot"
                        /></span>
                    </div>
                </div>
                ...
                <div class="actions" v-if="$store.getters.isSignedIn">
                    <button @click="menu" class="menu _button" ref="menu">
                        <fa :icon="faEllipsisH" />
                    </button>
                    <mk-follow-button
                        v-if="$store.state.i.id != user.id"
                        :user="user"
                        :inline="true"
                        :transparent="false"
                        :full="true"
            ...
            <div class="fields system">
                <dl class="field" v-if="user.location">
                    <dt class="name">
                        <fa :icon="faMapMarker" fixed-width />
                        {{ $t("location") }}
                    </dt>
                ...
                <dl class="field" v-if="user.birthday">
                    <dt class="name">
                        <fa :icon="faBirthdayCake" fixed-width />
                        {{ $t("birthday") }}
                    </dt>
                    <dd class="value">
                        {{
                ...
                <dl class="field">
                    <dt class="name">
                        <fa :icon="faCalendarAlt" fixed-width />
                        {{ $t("registeredDate") }}
                    </dt>
                    <dd class="value">
                        {{
            ...
            <div class="status">
                ...
                <router-link
                    :to="user | userPage('following')"
                    :class="{ active: $route.name === 'userFollowing' }"
                >
                    <b>{{ user.followingCount | number }}</b>
                    <span>{{ $t("following") }}</span>
                </router-link>
                <router-link
                    :to="user | userPage('followers')"
                    :class="{ active: $route.name === 'userFollowers' }"
                >
        ...
        <template v-if="$route.name == 'user'">
            <div class="pins">
                <x-note
                    ...
                    class="note"
                    :note="note"
                    @updated="pinnedNoteUpdated(note, $event)"
                    :key="note.id"
                    :detail="true"
                    :pinned="true"
                />
            ...
            <mk-container :body-togglable="true" class="content">
                <template #header
                    ><fa :icon="faImage" />{{ $t("images") }}</template
                >
                <div>
            ...
            <mk-container :body-togglable="true" class="content">
                <template #header
                    ><fa :icon="faChartBar" />{{ $t("activity") }}</template
                >
...
export default Vue.extend({
    ...
    metaInfo() {
        return {
            title: (this.user
                ? "@" + Vue.filter("acct")(this.user).replace("@", " | ")
                : null) as string,
        };
    },
    ...
    methods: {
        fetch() {
            Progress.start();
            this.$root
                .api("users/show", parseAcct(this.$route.params.user))
                .then((user) => {
                    this.user = user;
                })
                .catch((e) => {
        ...
        parallaxLoop() {
            this.parallaxAnimationId = window.requestAnimationFrame(
                this.parallaxLoop
            );
            this.parallax();
        },
